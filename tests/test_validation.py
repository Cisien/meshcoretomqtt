...
class TestValidateMeshcorePubkey:
    ...
    def test_63_chars_returns_none(self):
    ...
    def test_non_hex_g_returns_none(self):
        assert validate_meshcore_pubkey("G" * 64) is None

    def test_non_hex_z_returns_none(self):
    ...
    def test_all_fs_valid(self):
        assert validate_meshcore_pubkey("F" * 64) == "F" * 64

    def test_mixed_hex_valid(self):
...
class TestValidateEmail:
    ...
    def test_missing_dot_in_domain_returns_none(self):
        assert validate_email("user@examplecom") is None

    def test_starts_with_dot_returns_none(self):
        assert validate_email(".user@example.com") is None

    def test_starts_with_at_returns_none(self):
        assert validate_email("@example.com") is None

    def test_ends_with_dot_returns_none(self):
        assert validate_email("user@example.com.") is None

    def test_ends_with_at_returns_none(self):
        assert validate_email("user@") is None

    def test_double_dot_returns_none(self):
        assert validate_email("user@example..com") is None

    def test_space_returns_none(self):
        assert validate_email("us er@example.com") is None

    def test_empty_local_returns_none(self):
        # "@example.com" starts with @, caught by starts_with check
        assert validate_email("@example.com") is None

    def test_domain_too_short_returns_none(self):
        # domain "b" is 1 char < 3
        assert validate_email("a@b") is None

    def test_minimal_valid_3char_domain(self):
